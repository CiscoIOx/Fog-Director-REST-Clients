...
def device_specific_events(ip,token,device_ip):
	...
	for index in range(len(devices['data'])):
	    ...
	    if ip_addr==device_ip:
			url2="https://%s/api/v1/appmgr/devices/%s/events/" % (ip,device_id)
			headers = {'x-token-id':token,'content-type': 'application/json'}
			r=requests.get(url2,headers=headers,verify=False)
			print("Status code of fetching device events REST request %d") % r.status_code
			if r.status_code==200:
				events_list=json.loads((json.dumps(r.json())))
				for index in range(len(events_list['detailed_events'])):
					print events_list['detailed_events'][index]['eventType']
			else:
				print("there are no device specific events collected by the FD on this device")
...
device_specific_events(app_mgr_ip,token_id,device_ip)
