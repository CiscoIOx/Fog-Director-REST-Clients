...
sorting devices by metrics -- cpu , memory , network , disk 
...
def metric_sort_by(ip,token):
    url = "https://%s/api/v1/appmgr/devices/metrics?sortBy=cpu&sortOrder=asc&duration=d&offset=0&limit=5" % ip
    headers = {'x-token-id':token,'content-type': 'application/json'}
    r=requests.get(url,headers=headers,verify=False)
    print(r.status_code)
    print("metrics of the devices in your  FD")
...
metric_sort_by(app_mgr_ip,token_id)
