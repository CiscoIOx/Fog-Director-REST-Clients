...
Editing app configurtion from device details page of a device for a specific app.
i.e editing the package_config.ini for an app on a particular device
pre-req: app with package_config.ini should be deployed and started or stopped state on an device  
...
def edit_app_config_from_device_details_page(ip,token):
  ...
  #changing the package_config.ini for the app on this device 
  print("chnaging the configuration of the app %s on device %s") % (deployed_app,str(ip_addr))
  url3="https://%s/api/v1/appmgr/devices/%s/apps/%s/action" % (ip,device_id_confirmed,myappid)
  #this data payload "config" is with respect to the which i have used in this case - it can be different for yours
  data={"updateConfig":{"restart":"true","config":{"default":{"poll_frequency":"8","read_env":"true"},"sensor_control":{"debounce":"true","unit":"celcius"}}}}
  r = requests.post(url3,data=json.dumps(data),headers=headers,verify=False)
  print("Status code on changing the configuration of the app on device %s where this app is deployed: %d") % (str(ip_addr),r.status_code)
  response_action=json.loads((json.dumps(r.json())))
  print("reponse of the changing the configuration of the app from the device details page :%s") % response_action['response']
  if response_action['response']== "CONFIGURATION_UPDATED":
    print "CONFIGURATION_UPDATED"
  else:
    print "CONFIGURATION_UPDATE_failed"
...
edit_app_config_from_device_details_page(app_mgr_ip,token_id)
