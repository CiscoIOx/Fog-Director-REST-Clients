...
def count_resource_allocation_failed_devices(ip,token):
    app_name=raw_input("enter the name of the app")
    url = "https://%s/api/v1/appmgr/devices?offset=0&limit=10000&searchByApp=%s&searchByStatus=RESOURCE_ALLOCATION_FAILED" % (ip,app_name)
    ...
    print("number of devices which are in recource allocation failded state for this app :",len(devices['data']))
...
print "count of the devices which are in resource allocation failed state for an specific app "
count_resource_allocation_failed_devices(app_mgr_ip,token_id)
